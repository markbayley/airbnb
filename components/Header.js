...
function Header({placeholder, handleFilter, activeFilters}) {
  ...
  return (
    <header className="sticky top-0 z-50 grid grid-cols-3 bg-white shadow-md px-5 lg:px-10">
      ...
      <div onClick={()=> setIsOpened(!isOpened)} className="flex items-center justify-end lg:border-2 rounded-full my-3 md:shadow-sm">
        <input
          value={searchInput}
              ...
              <button
                ...
                className={
                  activeFilters &&  activeFilters?.includes("Favorites")
                    ? "p-1 ring-2 ring-red-400 bg-white hover:bg-red-500 rounded-full transition duration-200 ease-out shadow-lg "
                    : "p-1 ring-2 ring-red-400 bg-white hover:bg-red-500 rounded-full transition duration-200 ease-out shadow-lg "
                }
              >
              
                {activeFilters && activeFilters?.includes("Favorites") ? (
                  <HeartIcon className="h-4 cursor-pointer text-red-400" />
                ) : (
                  <HeartIconInactive className=" h-4 cursor-pointer text-red-400" />
                )}
              </button>
      ...
      {isOpened && (
        <div className="text-black flex flex-col col-span-3 mx-auto ">
         ...
         <div className="flex w-full justify-between pb-4 "><button onClick={resetInput} className="flex items-center justify-center bg-gray-400 rounded-full w-28 h-10 text-white">Close</button><button onClick={search} className="flex items-center justify-center bg-red-400 rounded-full w-28 h-10 text-white">Search</button></div>
        </div>
      )}
    </header>
  );
