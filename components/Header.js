      ...
      {isSearchBarOpen ? (
        ...
        <div className="flex  mx-auto md:hidden my-1 hover:shadow-lg">
          <div className="flex items-center ring-2 ring-red-400 rounded-l-full w-32 py-2">
            <input
              value={searchInput}
              onChange={(e) => { setSearchInput(e.target.value),  setIsSearchUpdated(true)}}
          ...
          <div className="flex items-center justify-center py-2 pl-3 bg-white  ring-2 ring-red-400">
            ...
            <input
              ...
              onChange={(e) => { setNumberOfGuests(e.target.value), setIsSearchUpdated(true)}}
              type="number"
              min={1}
          ...
          <div onClick={() => setIsSearchBarOpen(false)}><XMarkIcon className="h-6 p-1 shadow-lg bg-gray-400 text-white rounded-full ml-3"/></div>
        </div>
      ) : (
        <>
        {/* mobile menu */}
          ...
          <div>
            <div className="hidden md:flex items-center justify-center my-4 ">
              ...
              {/* desktop search icon */}
              <div className={  isSearchUpdated ? "ring-2 ring-red-400 rounded-r-full bg-red-400 px-2 hover:shadow-lg" : "ring-2 ring-red-400 rounded-r-full bg-red-400 px-2 hover:shadow-lg" }>
                <MagnifyingGlassIcon
                  onClick={search}
                  className={  isSearchUpdated ? "h-9  text-white  p-2 cursor-pointer hover:scale-125 transition duration-200 ease-out": "h-9  text-white  p-2 cursor-pointer hover:scale-125 transition duration-200 ease-out"}
                />
              </div>
            </div>
          </div>
...
{/* mobile search icon */}
          <div className="flex items-center justify-end text-gray-500 space-x-4">
            <div className="flex md:hidden ring-2 ring-red-400 rounded-full bg-red-400  ">
              <MagnifyingGlassIcon
            ...
            >
              {activeFilters && activeFilters?.includes("Favorites") ? (
               <div className="relative"><span className="absolute -right-2 flex justify-center -top-3 h-4 w-4 text-xs font-semibold text-white rounded-full bg-teal-500">{favorited.length}</span>  <HeartIcon className="h-6 cursor-pointer text-red-400 hover:scale-110 transition duration-200 ease-out hover:shadow-xl" /></div>
              ) : (
              <div className="relative"><span className="absolute -right-2 flex justify-center -top-3 h-4 w-4 text-xs font-semibold text-white rounded-full bg-teal-500">{favorited.length}</span>  <HeartIconInactive className="h-6 cursor-pointer text-red-400 hover:scale-110 transition duration-200 ease-out hover:shadow-xl" /></div>
              )}
            </div>
      ...
      {isDateRangePickerOpen && (
        <div className="text-black flex flex-col col-span-3 mx-auto ">
         ...
         <div className="bg-white rounded-b-md flex justify-end"> <button onClick={() => setIsDateRangePickerOpen(false) } className={numberOfDays ? "bg-teal-500 text-white h-auto rounded shadow-xl px-3 py-1 mr-3 mb-3": "bg-gray-500 text-white h-auto rounded-md shadow-xl px-3 py-1 mr-3 mb-3"}>Ok</button></div>
        </div>
      )}
    </header>
  );
